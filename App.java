...
public class App {
	...
	public static void main(String[] args) {
		...
		double boss = 0, notboss = 0;
		while (!ex.getPageHead().equals("Nothing")) {
			// nwscan = new Scanner(nonwork);
			// wscan = new Scanner(work);
			if (!ex.getPageHead().equals(ex.lasthead)) {
				ex.lasthead = ex.getPageHead();
				System.out.println(ex.lasthead);
				workcount = ex.work();
				noncount = ex.nonwork();
				...
				try {
					...
					writer.println(working + "," + notworking);
					writer.close();
	...
	//	test.add_vertex("Working+1", 2);
		...
		//sample.Roll_over(new Vertex<String>("Working",2), new Vertex<String>("Working+1",2));
